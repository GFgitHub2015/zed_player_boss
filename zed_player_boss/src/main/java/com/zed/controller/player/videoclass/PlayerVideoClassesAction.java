package com.zed.controller.player.videoclass;
...
@Controller("playerVideoClassesAction")
@Scope(value = "prototype")
public class PlayerVideoClassesAction extends BaseAction {
	...
	public String listYouTuBe() {
		try{
			mapList = youTuBeService.findCategoriesByRegionCodes();
			return SUCCESS;
		} catch (Exception ex) {
			if (ex instanceof AppValidationException) {
				AppValidationException e = (AppValidationException) ex;
	...
	public String add() {
		...
		try{
			...
			PlayerVideoClasses pvc = new PlayerVideoClasses();
	...
	public String update() {
		...
		try{
			...
			PlayerVideoClasses pvc = playerVideoClassesService.findById(cid);
			/*pvc.setClassKey(PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);*/
			pvc.setDescription(description);
			pvc.setSort(Integer.valueOf(sort));
			pvc.setFixed(fixed);
			pvc.setRecommended(recommended);
			pvc.setUpdater(sessionAdmin.getAdminId());
			pvc.setUpdateTime(new Timestamp(newdate.getTime()));
	...
	public String delete() {
		...
		try{
			...
			if (StringUtils.isNotBlank(id)) {
				PlayerVideoClasses pvc = playerVideoClassesService.findById(id);
		...
		try{
			...
			if (StringUtils.isNotBlank(classKey)) {
				if (StringUtils.isNotBlank(sort)) {
					if (StringUtils.isNotBlank(cid)) {
						sortFlag = playerVideoClassesService.getBySortAndClassKey(cid, Integer.valueOf(sort), PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
					}else{
						sortFlag = playerVideoClassesService.getBySortAndClassKey(null, Integer.valueOf(sort), PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
					}
				}else{
					sortFlag = playerVideoClassesService.getByClasskey(PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
				}
			...
			if (StringUtils.isNotBlank(classKey)) {
				if (StringUtils.isNotBlank(description)) {
					if (StringUtils.isNotBlank(cid)) {
						descriptionFlag = playerVideoClassesService.getByDescriptionAndClassKey(cid, description, PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
					}else{
						descriptionFlag = playerVideoClassesService.getByDescriptionAndClassKey(null, description, PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
					}
				}else{
					descriptionFlag = playerVideoClassesService.getByClasskey(PlayerVideoClasses.Prefix.YOUTUBE.getKey()+classKey);
				}
