			...
			if (!form.data('jqv') || form.data('jqv') == null ) {
				...
				$(document).on("click", ".formError", function() {
					$(this).fadeOut(150, function() {
						...
						$(this).parent().find('.tips').css('display','inline');
		...
		*/
		_validateField: function(field, options, skipAjaxValidation) {
           ...
           if (!options.validateNonVisibleFields && (field.is(":hidden") && !options.prettySelect || field.parent().is(":hidden")))
			...
			if(!options.isError){
				field.parent().find('.tips').css('display','inline');
			}
			 ...
			 if (!options.isError && !methods._checkAjaxFieldStatus(field.attr("id"), options)) {
				 $.ajax({
					 ...
					 success: function(json) {
						 ...
						 var errorFieldId = json.data[0];
//						 var errorFieldId = json[0];
						 //var errorField = $($("#" + errorFieldId)[0]);
//						 var errorField = $("#"+ errorFieldId).eq(0);
						 var errorField = $($("input[id='" + errorFieldId +"']")[0]);
						 if (errorField.length == 0) {
							 errorField = $($("select[id='" + errorFieldId +"']")[0]);
						 }

						 // make sure we found the element
						 if (errorField.length == 1) {
							 var status = json.data[1];
//							 var status = json[1];
...
//							 var msg = json[2];
		...
		*/
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
			...
			field.parent().find('.tips').css('display','none');
