...
<body>
	...
	<form id = "form" action="<s:url action="list" />" method="post" name="form">
		<ul class="seachform">
				<li>
					<label>搜索</label>
					<input id="searchkey" name="searchkey" type="text" class="scinput form-control" maxlength="128" value = "${searchkey}"/>
				</li>
				<li>
					<label>状态</label>
			...
			<tbody>
				<s:iterator value="page.result" var="list">
					<tr>
								...
								<td>
									<c:choose>
						    			<c:when test="${list.sex == '3'}">
						    				男
						    			</c:when>
						    			<c:when test="${list.sex == '4'}">
						    				女
						    			</c:when>
						    			<c:when test="${list.sex == '2'}">
						    				未知
						    			</c:when>
								...
								<td>
									<c:choose>
						    			<c:when test="${list.user_type == '1'}">
						    				普通用户
						    			</c:when>
						    			<c:when test="${list.user_type == '2'}">
						    				游客
						    			</c:when>
						    			<c:otherwise>
						    			</c:otherwise>
								...
								<td>									
									<s:iterator value="buttonMenuList" var="buttonRight">
										<s:if test="%{#buttonRight.resourceLevel == 5}">
											<s:if test="%{#buttonRight.resourceKey == 'menu_account_table_status_righ'}">
									...
									<a href="javascript:reSetPasswdOne('/boss/account/updatePassWordPage.action?userId=','${list.user_id}')"  class="tablelink">修改密码</a>
								</td>
						</tr>				
				</s:iterator>
			</tbody>
