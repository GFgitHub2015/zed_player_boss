...
function platformDetail(adPlatform){
	...
	var url = '<s:url action="listPlatformStatAjax.action" />';
	var data ={"channel" : adPlatform};
        ...
        success:function(data){
			if(data&&data.data){
           	 ...
           	 for (var index = 0; index < resultJson.length; index++) {
           		...
           		trHtml +="<td><a href=\"javascript:showChart('"+resultJson[index].adPlatform+"','"+adPlatform+"')\"  <i class='fa fa-line-chart'></i></a></td>";
           		trHtml +="<td>--</td>";
           		trHtml +="</tr>";
			}
...
function changeDataType(obj){
	var url = '<s:url action="listPlatformStat.action" />';
	var dtype = $(obj).val();
	if(dtype==1){
...
<div class="mainindex">
    <form id = "form" action="<s:url action="listAppStat" />" method="post" name="form">
        ...
        <table class="tablelist">
            ...
            <tr>
                    ...
                    <td>--</td>
                    <td>--</td>
                    <td><a href="javascript:showChart()"  <i class="fa fa-line-chart"></i></a></td>
                    <td>--</td>
                </tr>
            <s:iterator value="page.result" var="list" status="st">
                <tr id="${list.uid }-total">
                    <td><a href="javascript:appDetail('${list.uid }')"  ><span class="fbold">${list.adPlatform } </span></a><a href="javascript:platformDetail('${list.uid }')"  ><i id="${list.uid }-icon" class="fa fa-chevron-right"></i></a></td>
                    ...
                    <td>${list.ecpm }</td>
                    <td><a href="javascript:showChart('','${list.uid }')"  <i class="fa fa-line-chart"></i></a></td>
                    <td>
                    	<%-- <span title='双击修改' class='scalespan'  ondblclick='showChangeScaleInput(this)'>${list.scale*100 }%</span>
           				<input channel='${list.uid }' oldvalue="${list.scale*100 }"   type='number' class='scaleinput' style='display:none;'  value='${list.scale*100 }'> --%>
           				<a appname='${list.adPlatform }' channel='${list.uid }'  id='${list.adPlatform }-${list.uid }-scale' class='scalespan'   ><i class='fa fa-file-text'></i></a>
           			</td>
                </tr>
            </s:iterator>
            </tbody>
