...
function changeType(){
	...
	if($("input[name='playerSliderShow.type']:eq(0)").attr("checked")){
		...
		$("#videovalue").removeClass('validate[required,minSize[1],maxSize[1024]]');
		...
		$("#videovalue").removeClass('validate[required,minSize[1],maxSize[1024]]');
		$("#videovalue").addClass('validate[required,minSize[1],maxSize[1024],ajax[isFileIdExist]]');
		$("#link").hide();
		$("#video").show();
	}else if($("input[name='playerSliderShow.type']:eq(2)").attr("checked")){
		$("#linkvalue").removeAttr("value");
		$("#linkvalue").removeClass('validate[required,minSize[1],maxSize[1024],custom[url]]');
		$("#videovalue").removeClass('validate[required,minSize[1],maxSize[1024],ajax[isFileIdExist]]');
...
//     }
    ...
    var data = {"areaCode": areaCode,"sort":sort};
    ...
    $.ajax({
        ...
        success:function(data){
       	 if(data&&data.data){
       		layer.open({
      		  content: data.areaCodeNames+'地区展示位已存在，是否替换现有展示位轮播图？',
      		  btn: ['确定', '取消'],
    ...
    <ul class="forminfo">
	    ...
	    <li>
	    	...
	    	<cite>
	    		...
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerSliderShow.type" type="radio" value="1"/>&nbsp;&nbsp;gbox视频
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerSliderShow.type" type="radio" value="2"/>&nbsp;&nbsp;youtube视频
	    	</cite>
    	</li>
    	 <li id="link">
