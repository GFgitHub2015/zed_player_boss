		...
		<ul class="seachform">
				...
				<li>
					...
					<select name="areaCode" id="areaCode" class="dayinput form-control" style="width:150px;">
						...
						<option value="62" <c:if test = "${areaCode == '62'}">selected="selected"</c:if>>印度尼西亚</option>
						<option value="66" <c:if test = "${areaCode == '66'}">selected="selected"</c:if>>泰国</option>
						<option value="84" <c:if test = "${areaCode == '84'}">selected="selected"</c:if>>越南</option>
						<option value="86" <c:if test = "${areaCode == '86'}">selected="selected"</c:if>>中国</option>
						<option value="91" <c:if test = "${areaCode == '91'}">selected="selected"</c:if>>印度</option>
						<option value="852" <c:if test = "${areaCode == '852'}">selected="selected"</c:if>>香港</option>
					</select>
				</li>
				<li>
					<label>&nbsp;</label>
								...
								<td>													
									<s:iterator value="buttonMenuList" var="buttonRight">
										<s:if test="%{#buttonRight.resourceLevel == 5}">
											<s:if test="%{#buttonRight.resourceKey == 'menu_player_content_kw_table_status_righ'}">
												<s:if test="#list.status == 1">
													<a href="javascript:${buttonRight.functionName}('${projectURLPrefix}${buttonRight.resourceUrl}?status=0&hotKeyWordId=','${list.keyword_id}')"  class="tablelink">禁用</a>
												</s:if>
												<s:elseif test="#list.status == 0">
													<a href="javascript:${buttonRight.functionName}('${projectURLPrefix}${buttonRight.resourceUrl}?status=1&hotKeyWordId=','${list.keyword_id}')"  class="tablelink">启用</a>
												</s:elseif>
											</s:if>
											<s:else>
