...
function changeDataType(obj){
	var url = '<s:url action="list.action" />';
	var dtype = $(obj).val();
	if(dtype==1){
...
function showChart( channel){
	var url = '<s:url action="listDateDetail.action" />';
	$("#channel").val(channel);
	$("#form").attr("action",url);
	document.form.submit();
}
...
<div class="mainindex">
    <form id = "form" action="<s:url action="listDate" />" method="post" name="form">
        ...
        <table class="tablelist">
            ...
            <tr>
                ...
                <td>总和</td>
                <td>${shareVideoStat.sharePv }</td>
                <td>${shareVideoStat.shareUv }</td>
                <td>${shareVideoStat.downloadCount }</td>
                <td>${shareVideoStat.shareCount }</td>
            ...
            <s:iterator value="page.result" var="list" status="st">
                <tr>
                    ...
                    <td><a href="javascript:showChart('${list.channel }')"  <i class="fa fa-line-chart"></i></a></td>
                </tr>
            </s:iterator>
            </tbody>
        </table>
