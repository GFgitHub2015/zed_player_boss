...
function clearForm() {
	window.location = '<s:url action="updatePage" />?infoId=${playerPromotionInfo.id}';
}
...
<s:token name="submittedToken"/>
	...
	<input type="hidden" name="playerPromotionInfo.id" id="playerPromotionInfo.id" value="${playerPromotionInfo.id}"/>
	<input type="hidden" name="playerPromotionInfo.content" id="playerPromotionInfo.content" value="${playerPromotionInfo.content}"/>
    <div class="formbody">
    <div class="formtitle"><span>修改活动推送信息</span></div>
	    ...
	    <li><label style="width:120px;">开始时间<font class = "require">*</font></label>
	    	<input name="playerPromotionInfo.startTime" id="startTime" style="width:150px;" class="Wdate" value="${playerPromotionInfo.startTime}" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
	    </li>
	    <li><label style="width:120px;">结束时间<font class = "require">*</font></label>
	    	<input name="playerPromotionInfo.endTime" id="endTime" style="width:150px;" class="Wdate" value="${playerPromotionInfo.endTime}" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
	    </li>
	    <li>
    	...
    	<li>
			...
			<select name="playerPromotionInfo.countryCode" id="playerPromotionInfo.countryCode" class="dayinput form-control" style="width:500px;">
				<c:if test="${not empty playerCountryList}">
					<s:iterator value="playerCountryList" var="playerCountry">
						<option value="<s:property value="#playerCountry.countryCode"/>" <s:if test="%{#playerCountry.countryCode == playerPromotionInfo.countryCode}">selected="selected"</s:if>>
    	...
    	<div id="recommendFlag" style="display: none;">
		    <li><label style="width:120px;">上架开始时间<font class = "require">*</font></label>
		    	<input name="playerPromotionInfo.upTime" id="upTime" style="width:150px;" class="Wdate validate[required]" value="${playerPromotionInfo.upTime}" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
		    </li>
		    <li><label style="width:120px;">上架结束时间<font class = "require">*</font></label>
		    	<input name="playerPromotionInfo.downTime"  id="downTime"  style="width:150px;" class="Wdate validate[required]" value="${playerPromotionInfo.downTime}" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
		    </li>
		    <li>
