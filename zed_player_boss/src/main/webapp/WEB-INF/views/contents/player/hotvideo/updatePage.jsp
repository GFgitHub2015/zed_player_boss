...
<s:token name="submittedToken"/>
    ...
    <div class="formtitle"><span>修改热门影片推荐</span></div>
        ...
        <li>
            ...
            <input type="radio" name="hotVideo.tagType" value="0"  <c:if test="${hotVideo.tagType == 0 ||empty hotVideo.tagType }">checked="checked"</c:if>/>自动判定
            <input type="radio" name="hotVideo.tagType" value="1"  <c:if test="${hotVideo.tagType == 1}">checked="checked"</c:if>/>最热
            <input type="radio" name="hotVideo.tagType" value="2"  <c:if test="${hotVideo.tagType == 2}">checked="checked"</c:if>/>最新
        </li>
	    ...
	    <li>
			...
			<select name="hotVideo.countryCode" id="hotVideo.countryCode" class="dayinput form-control" style="width:500px;">
				<c:if test="${not empty playerCountryList}">
					<s:iterator value="playerCountryList" var="playerCountry">
						<option value="<s:property value="#playerCountry.countryCode"/>" <s:if test="%{#playerCountry.countryCode == hotVideo.countryCode}">selected="selected"</s:if>>
