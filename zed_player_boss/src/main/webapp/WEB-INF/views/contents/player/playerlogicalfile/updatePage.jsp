...
function clearForm() {
	window.location = '<s:url action="updatePage" />?fileId=${playerLogicalFile.fileId}';
}
...
<form  method="post" name="form" id="form" enctype="multipart/form-data"  action="<s:url action="update"/>" onsubmit="return false;">
	...
	<input type="hidden" name="playerLogicalFile.fileId" id="playerLogicalFile.fileId" value="${playerLogicalFile.fileId}"/>
	<input type="hidden" name="playerLogicalFile.creater" id="playerLogicalFile.creater" value="${playerLogicalFile.creater}"/>
	<input type="hidden" name="playerLogicalFile.createTime" id="playerLogicalFile.createTime" value="${playerLogicalFile.createTime}"/>
	<input type="hidden" name="playerLogicalFile.sourceFileId" id="sourceFileId"  value="${playerLogicalFile.sourceFileId }"/>
	<input type="hidden" name="playerLogicalFile.shareStatus" id="shareStatus"  value="${playerLogicalFile.shareStatus }"/>
	<input type="hidden" name="playerLogicalFile.isFile" id="isFile"  value="${playerLogicalFile.isFile }"/>
	<input type="hidden" name="playerLogicalFile.userId" id="userId"  value="${playerLogicalFile.userId }"/>
	<input type="hidden" name="playerLogicalFile.fileSize" id="fileSize"  value="${playerLogicalFile.fileSize }"/>
	<input type="hidden" name="playerLogicalFile.sourceType" id="sourceType"  value="${playerLogicalFile.sourceType }"/>
	<input type="hidden" name="playerLogicalFile.filePath" id="filePath"  value="${playerLogicalFile.filePath }"/>
	<input type="hidden" name="playerLogicalFile.parentFileId" id="parentFileId"  value="${playerLogicalFile.parentFileId }"/>
    <div class="formbody">
    <div class="formtitle"><span>用户资源信息</span></div>
	    <ul class="forminfo">
	    <li>
	    	<label style="width:120px;">文件名<font class = "require">*</font></label>
	    	<input name="playerLogicalFile.fileName" id="playerLogicalFile.fileName" type="text" class="dfinput validate[required,minSize[1],maxSize[255]]" value="${playerLogicalFile.fileName}" maxlength="255"/><i class = "tips">请输入文件名</i>
	    </li>
	    <li>
	    	<label style="width:120px;">状态</label>
	    	<cite>
	    		<input name="playerLogicalFile.status" type="radio" value="0" <c:if test="${playerLogicalFile.status eq '0'}">checked="checked"</c:if>/>&nbsp;&nbsp;禁用
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerLogicalFile.status" type="radio" value="1" <c:if test="${playerLogicalFile.status eq '1'}">checked="checked"</c:if>/>&nbsp;&nbsp;启用		
	    	</cite>
	    </li>    
	    <li>
	    	<label style="width:120px;">播放模式</label>
	    	<cite>
	    		<input name="dimension" type="radio" value="2" <c:if test="${dimension eq '2'}">checked="checked"</c:if>/>&nbsp;&nbsp;2D
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="dimension" type="radio" value="3" <c:if test="${dimension eq '3'}">checked="checked"</c:if>/>&nbsp;&nbsp;3D
	    	</cite>
	    </li>   
	    <li>
	    	<label style="width:520px;">字幕信息&nbsp;&nbsp;&nbsp;(zh-中文&nbsp;|&nbsp;en-英语&nbsp;|&nbsp;th-泰语&nbsp;|&nbsp;vi-越南&nbsp;|&nbsp;hi-印度&nbsp;|&nbsp;in-印度尼西亚&nbsp;|&nbsp;ms-马来语)</label>
			<input type="hidden" name="resId" id="resId" value="${resId}" />
			<input type="hidden" name="delSubtitleIds" id="delSubtitleIds" />
			<div class="btn_right">
				<button type="button" class="btn"  id="addSubtitle" value="新增字幕">新增字幕</button>
			</div>
