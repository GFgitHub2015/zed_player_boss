...
//         console.log("pushTime show");
    }else if($("[id='playerTaskParams.picked']").val() == "0"){
...
// 		console.log("pushTime hiden");
    }else{
    	$("[id='playerTaskParams.pushTime']").hide();
    	
//     	console.log("all hiden");
    }
}
	...
	$("[id='playerTaskParams.target']").on('change',function(){
		...
		switch (targetValue){
			...
			case '3':
				...
				$('.js-region-select').css({"display":'none'}).find("[id='playerTaskParams.topicType']").removeClass('validate[required]');
				$('.js-user-id').css({"display":'inline-block'}).find("[id='playerTaskParams.userId']").addClass('validate[required,minSize[1],maxSize[32],ajax[isUserExist]]');
			break;	
			default:
				$('.js-region-select').css({"display":'none'}).find("[id='playerTaskParams.topicType']").removeClass('validate[required]');
				$('.js-user-id').css({"display":'none'}).find("[id='playerTaskParams.userId']").removeClass('validate[required,minSize[1],maxSize[32],ajax[isUserExist]]');
...
function clearForm() {
	window.location = '<s:url action="updatePage" />?taskId=${playerTaskParams.taskId}';
}
...
<form  method="post" name="form" id="form" action="<s:url action="update"/>" onsubmit="return false;">
	<input type="hidden" name="playerTaskParams.taskId" id="playerTaskParams.taskId" value="${playerTaskParams.taskId}"/>
	<input type="hidden" name="playerTaskParams.version" id="playerTaskParams.version" value="${playerTaskParams.version}"/>
	<input type="hidden" name="playerTaskParams.channel" id="playerTaskParams.channel" value="${playerTaskParams.channel}"/>
	<input type="hidden" name="playerTaskParams.language" id="playerTaskParams.language" value="${playerTaskParams.language}"/>
    <div class="formbody">
    <div class="formtitle"><span>修改消息推送</span></div>
	    <ul class="forminfo">
	    <li>
	    	<label style="width:120px;">消息标题<font class = "require">*</font></label>
	    	<input name="playerTaskParams.title" id="playerTaskParams.title" type="text" class="dfinput validate[required,minSize[1],maxSize[150]]" maxlength="150" value="${playerTaskParams.title}"/><i class = "tips">请输入消息标题</i>
	    </li>
		<li>
	    	<label style="width:120px;">消息内容<font class = "require">*</font></label>
	    	<input name="playerTaskParams.content" id="playerTaskParams.content" type="text" class="dfinput validate[required,minSize[1],maxSize[2500]]" maxlength="2500" value="${playerTaskParams.content}"/><i class = "tips">请输入消息内容</i>
	    </li>
	     <li>
	    	<label style="width:120px;">消息备注<font class = "require">*</font></label>
	    	<input name="playerTaskParams.description" id="pushTask.description" type="text" class="dfinput validate[required,minSize[1],maxSize[250]]" maxlength="250" value="${playerTaskParams.description}"/><i class = "tips">请输入消息备注</i>
	    </li>
	    <li>
	    	<label style="width:120px;">图标<font class = "require">*</font></label>
	    	<cite>
	    		<input name="playerTaskParams.icon" type="radio" value="0" <c:if test = "${playerTaskParams.icon == '0'}">checked="checked"</c:if>/>&nbsp;&nbsp;默认
	    		<%-- &nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerTaskParams.icon" type="radio" value="1" <c:if test = "${playerTaskParams.icon == '1'}">checked="checked"</c:if>/>&nbsp;&nbsp;自定义 --%>
	    	</cite>
    	</li>
    	<li>
	   ...
	   <li class="js-target-user">
	    	...
	    	<select name="playerTaskParams.target" id="playerTaskParams.target" class="dayinput form-control" style="width:150px;display:inline-block;margin-right:30px;" isset='${playerTaskParams.target}'>
			...
			<div class="js-region-select">
				<select name="playerTaskParams.topicType" id="playerTaskParams.topicType" class="dayinput form-control" style="width:150px;vertical-align: top;display:inline-block;margin-right:20px;" isset='${playerTaskParams.topicType}'>
					 		...
					 		<div class="dd-wrap">
					 			...
					 			<c:set var="flag" value="flase" scope="page"/>
						 			<c:forTokens items="${playerTaskParams.language}" varStatus="status" delims="," var="l">
							 			...
							 			<dd><input name="language" type="checkbox" class="check-item" value="${language}" <c:if test="${flag}">checked</c:if>><span>${language}</span></dd>
					 			</s:iterator>
					 		</div>	 		
					 	</dl>
					</div>
						...
						<dl class="dl-list" >
					 		...
					 		<div class="dd-wrap">
					 			<s:iterator value="channelList" var="channel">
						 			...
						 			<dd><input name="channel" type="checkbox" class="check-item" value="${channel}" <c:if test="${flag}">checked="checked"</c:if>><span>${channel}</span></dd>
					 			</s:iterator>
					 		</div>
						...
						<dl class="dl-list">
					 		...
					 		<div class="dd-wrap">
					 			<s:iterator value="appVersionList" var="appVersion">
					 				...
					 				<dd><input name="version" type="checkbox" class="check-item" value="${appVersion}" <c:if test="${flag}">checked="checked"</c:if>><span>${appVersion}</span></dd>
					 			</s:iterator>
					 		</div>
					 	</dl>
					</div>
    	...
    	<li class="js-user-id" style="display:none;">
			...
			<input name="playerTaskParams.userId" id="playerTaskParams.userId" type="text" class="dfinput" maxlength="32" value="${playerTaskParams.userId}"/><i class = "tips">请输入用户ID</i>
    	</li>
	    <li>
	    	<label style="width:120px;">后续动作<font class = "require">*</font></label>
	    	<cite>
	    		<input name="playerTaskParams.operation" type="radio" value="0" <c:if test = "${playerTaskParams.operation == '0'}">checked="checked"</c:if>/>&nbsp;&nbsp;打开应用
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerTaskParams.operation" type="radio" value="1" <c:if test = "${playerTaskParams.operation == '1'}">checked="checked"</c:if>/>&nbsp;&nbsp;打开页面
	    	</cite>
	    	
    	</li>
	    <li class="js-video-type" style="display:none;">
	    	<label style="width:120px;">类型<font class = "require">*</font></label>
	    	<select name="playerTaskParams.type" id="playerTaskParams.type" class="dayinput form-control" style="width:150px;" isset="${playerTaskParams.type}">
	    ...
	    <li class="js-video-url" style="display:none;">
	    	...
	    	<input name="playerTaskParams.url" id="playerTaskParams.url" type="text" class="dfinput" maxlength="250" value="${playerTaskParams.url}"/><i class = "tips">请输入url，例：http://xxx</i>
	    </li>
	    <li class="js-video-id" style="display:none;">  	
	    	<label style="width:120px;">影片ID<font class = "require">*</font></label>
	    	<input name="playerTaskParams.fileId" id="playerTaskParams.fileId" type="text" class="dfinput" maxlength="250" value="${playerTaskParams.fileId}"/><i class = "tips">请输入影片ID</i>
	    </li>
	    <li>
	    	<label style="width:120px;">提醒方式<font class = "require">*</font></label>
	    	<cite>
	    		<input name="playerTaskParams.reminderType" type="radio" value="0" <c:if test = "${playerTaskParams.reminderType == '0'}">checked="checked"</c:if>/>&nbsp;&nbsp;全部
	    		&nbsp;&nbsp;&nbsp;&nbsp;
	    		<input name="playerTaskParams.reminderType" type="radio" value="1" <c:if test = "${playerTaskParams.reminderType == '1'}">checked="checked"</c:if>/>&nbsp;&nbsp;震动
	    		<input name="playerTaskParams.reminderType" type="radio" value="2" <c:if test = "${playerTaskParams.reminderType == '2'}">checked="checked"</c:if>/>&nbsp;&nbsp;响铃
	    	</cite>
    	</li>
 		<li>
	    	...
	    	<input  name="playerTaskParams.pushTime" id="playerTaskParams.pushTime" class="Wdate" style="width: 200px;"  readonly="readonly" onclick="getMinDate();" value="${playerTaskParams.pushTime}"/>
	    </li>
	    <li><label>&nbsp;</label>
